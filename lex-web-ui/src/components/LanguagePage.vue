<template>
  <div class="language-page">
    ...
    <p class="subtitle">SELECT YOUR PREFERRED LANGUAGE FROM THE LIST BELOW TO GET STARTED.</p>
...
export default {
  ...
  methods: {
    ...
    setLanguage(language) {
      const translations = this.$store.state.config.translations;
      const localeCode = language === 'English' ? 'en_US' : (translations[language] ? translations[language].code : language);
      
      if (this.availableLocaleCodes.includes(localeCode)) {
        const revised = [localeCode, ...this.availableLocaleCodes.filter(code => code !== localeCode)];
        ...
        console.warn(`Locale code ${localeCode} not available in bot locales.`);
      }
