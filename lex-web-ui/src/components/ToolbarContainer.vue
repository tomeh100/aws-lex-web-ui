  ...
  >
    ...
    <v-menu v-if="showToolbarMenu">
      ...
      <v-list>
        ...
        <v-list-item v-if="isSaveChatEnabled">
          <v-list-item-title @click="saveChat" :aria-label="saveChatAriaLabel">
            <v-icon>
              {{ saveTranscriptIcon }}
            </v-icon>
            {{ saveTranscriptLabel }}
        ...
        <v-list-item v-if="isEndChatEnabled">
          <v-list-item-title @click="endChat" :aria-label="endChatAriaLabel">
            <v-icon>
              {{ endChatIcon }}
            </v-icon>
            {{ endChatLabel }}
    ...
    >
      ...
      <span class="ps-1">{{ currentTranslatedLanguageName }}</span>
    </v-btn>
    <v-btn
    ...
    >
      <v-icon>
        {{ isUiMinimized ? 'chat' : minimizeIcon }}
      </v-icon>
    </v-btn>
  </v-toolbar>
  <language-selector
...
*/
  ...
  export default {
    ...
    data() {
      return {
        items: [
          ...
          { title: 'Save Transcript', icon: 'save' },
          { title: 'End Chat', icon: 'exit_to_app' },
        ],
    ...
    computed: {
      ...
      isLexProcessing() {
        return (
          this.$store.state.isBackProcessing ||
          this.$store.state.lex.isProcessing
        )
      },
      ...
      isSaveChatEnabled() {
        return this.$store.state.config.ui.saveChatEnabled
      },
      isEndChatEnabled() {
        return this.$store.state.config.ui.endChatEnabled
      },
      currentLanguage() {
        return this.$store.state.currentLanguage || 'English'
      },
      currentTranslatedLanguageName() {
        const translations = this.$store.state.config.translations
        return translations[this.currentLanguage]?.translatedName || 'English'
      },
      translations() {
      ...
      minimizeIcon() {
        return this.$store.state.config.ui.minimizeIcon || 'arrow_drop_down'
      },
    },
    methods: {
      setLocale(l) {
      ...
      isValidHelpContentForUse() {
        ...
        return (
          ...
          ((helpContent[localeId].text &&
            helpContent[localeId].text.length > 0) ||
            (helpContent[localeId].markdown &&
              helpContent[localeId].markdown.length > 0))
        )
      },
            ...
            genericAttachments: [
              {
                ...
                attachmentLinkUrl:
                  helpContent[localeId].responseCard.attachmentLinkUrl,
      ...
      onLanguageChange(newLanguage) {
        ...
        this.$store.dispatch(
          'postTextMessage',
          this.messageForChangeLanguage(newLanguage)
        )
      },
      ...
      messageForEndChatContent() {
        return {
          text:
            this.translations[this.currentLanguage].endChat.messageContent ||
            'End Chat',
          type: 'human',
        }
      },
